...
function isDisabledHint<T>(hint: T | false | undefined): hint is false {
  return hint === false
}
...
export function applyCacheControlMax(
  ...
  if (Array.isArray(sys) && sys.length > 0) {
    if (!isDisabledHint(hints?.system)) {
  ...
  if (Array.isArray(msgs) && msgs.length > 0) {
    ...
    if (!isDisabledHint(hints?.messages)) {
